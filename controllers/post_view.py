...
def generateCommentTree(commentid, postid, opUsername): #Returns the html of the comment tree starting at commentid recursively. If commentid = 0, start with the post itself
	...
	for comment in comments:
		...
		htmlToReturn += comment_template_html.format(str(comment["commentid"]), str(comment["comment"]), str(comment["commentid"]), str(id), str(comment["commentid"]), str(comment["commentid"]),display,comment["dateCreated"].strftime("%m/%d/%y %I:%M%p"),childrenHtml)
