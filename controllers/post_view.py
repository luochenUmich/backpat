...
def generateCommentTree(commentid, postid, opUserid): #Returns the html of the comment tree starting at commentid recursively. If commentid = 0, start with the post itself
    ...
    while(comment):
        ...
        htmlToReturn += render_template('comment_template.html', username=generateUniquename(comment["userid"], postid), dateCreated=str(comment["dateCreated"]), commentid=comment["commentid"], description=comment["comment"], comment_child_html=childrenHtml)
        comment = cursor.fetchone()
    conn.close()
    cursor.close()
    return htmlToReturn
